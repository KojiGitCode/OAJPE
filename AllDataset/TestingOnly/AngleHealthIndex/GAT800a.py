...
def btn9_click(counter):
    ...
    ### For GitHub ###
    print("var86: ",var86.get())
    var26.set(var56.get()) # synchronize the selected index between Training and Testing tabs
    print("var26: ",var26.get())
    print("var56: ",var56.get())
    args_Ainx = var56.get()
    print("args_Ainx: ", args_Ainx)
                ...
                if   int(args_Ainx) == 0:
                    ...
                    for kk in range (NumGraph):
                        ...
                        if   180.0 > LBL0[kk] and LBL0[kk] >= 90.0:
                            ...
                            OUTPUT0[kk] = 6
                        elif 25.0  > LBL0[kk] and LBL0[kk] >= 15.0:
                            OUTPUT0[kk] = 7
                        elif 15.0  > LBL0[kk] and LBL0[kk] >= 10.0:
                            OUTPUT0[kk] = 8
                        elif 10.0  > LBL0[kk] and LBL0[kk] >=  7.0:
                            OUTPUT0[kk] = 9
                        elif  7.0  > LBL0[kk] and LBL0[kk] >=  4.0:
                            OUTPUT0[kk] = 10
                        elif  4.0  > LBL0[kk] and LBL0[kk] >=  2.0:
                            OUTPUT0[kk] = 11
                        elif  2.0  > LBL0[kk] and LBL0[kk] >=-90.0:
                            OUTPUT0[kk] = 12
    ...
    class MLP4model(nn.Module): #MLP4 model
        ...
        def forward(self, data):
            ...
            if int(args_logits) > 0:
                ...
                y = F.log_softmax(self.fc4(x),  dim=1)
            return  y
    class MLPmodel(nn.Module): #MLP5 model
        def __init__(self):
            super().__init__()
            node_feature = dataset.num_node_features # 6 => 5
            num_classes  = dataset.num_classes       # 10
    ...
    if os.path.exists(os.path.join(temp, "ContingencyCase.prn")):
        ...
        if   NumBus == 14 and int(args_Ainx) < 5: # Finx (graph classificataion)
            ...
            #est_loader   = DataLoader(test_dataset,  batch_size=int(Data10p/21), shuffle=False, drop_last=True) #2520
            test_loader   = DataLoader(test_dataset,  batch_size=Data10p,
                    shuffle=False, drop_last=False) #827
    ...
    if os.path.exists(os.path.join(temp, "ContingencyCase.prn")):
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                test_acc   += torch.sum(pred == data.y).item()# / dataset.num_classes
                cmp_test_cls.append([pred,data.y])
                test_f1  += f1_score_multiclass(pred, data.y, dataset.num_classes)
                test_mcc += mcc_multiclass     (pred, data.y, dataset.num_classes)
            if int(args_Ainx) < 5: # Finx (graph classificataion)
...
#ile_path22 = os.path.join(os.getcwd(),'Sample_Datasets','Freq','TestingDataset')
file_path22 = os.path.join(os.getcwd(),'Sample_Datasets','Angl','TestingDataset2') #TestingDataset: 6 Labels, TestingDataset2: 14 Labels
tee2("Input folder path is ", file_path22, ConsoleFile)
global txt_26, file_path52 # Node/Bus feature file
#ile_path52 = os.path.join(os.getcwd(),'Sample_Datasets','Freq','node_feature_default.lst')
...
#ile_path53 = os.path.join(os.getcwd(),'Sample_Datasets','Freq','branch_feature_default.lst')
...
if   args.mdl == "GCN5":
    if args.enc == "Y":
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN5H0LR0.001Enc1.pth')
    else:
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN5H0LR0.001Enc0.pth')
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'EdgeNeuron.npy'), np.array(32)) # Number of Neurons for Edges
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'MdlType.npy'), np.array(1))#0:"GCN2",1:"GCN5",2:"GCN6",3:"GCN3",4:"SGA2",5:"SGA3",6:"SGA4",7:"SGA5",8:"GCN4",9:"MLP"
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'Neuron.npy'), np.array(192))     # Number of Neurons for Nodes
    np.save(os.path.join(temp, 'GATHead.npy'), np.array(0)) # Number of Heads of GAT Model
elif args.mdl == "GCN6":
    if args.enc == "Y":
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN6H0LR0.001Enc1.pth')
    else:
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN6H0LR0.001Enc0.pth')
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'EdgeNeuron.npy'), np.array(32)) # Number of Neurons for Edges
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'MdlType.npy'), np.array(2))#0:"GCN2",1:"GCN5",2:"GCN6",3:"GCN3",4:"SGA2",5:"SGA3",6:"SGA4",7:"SGA5",8:"GCN4",9:"MLP"
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'Neuron.npy'), np.array(192))     # Number of Neurons for Nodes
    np.save(os.path.join(temp, 'GATHead.npy'), np.array(0)) # Number of Heads of GAT Model
elif args.mdl == "GCN2":
    if args.enc == "Y":
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN2H0LR0.001Enc1.pth')
    else:
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN2H0LR0.001Enc0.pth')
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'EdgeNeuron.npy'), np.array(32)) # Number of Neurons for Edges
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'MdlType.npy'), np.array(0))#0:"GCN2",1:"GCN5",2:"GCN6",3:"GCN3",4:"SGA2",5:"SGA3",6:"SGA4",7:"SGA5",8:"GCN4",9:"MLP"
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'Neuron.npy'), np.array(192))     # Number of Neurons for Nodes
    np.save(os.path.join(temp, 'GATHead.npy'), np.array(0)) # Number of Heads of GAT Model
elif args.mdl == "GCN4":
    if args.enc == "Y":
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN4H0LR0.001Enc1.pth')
    else:
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN192GCN4H0LR0.001Enc0.pth')
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'EdgeNeuron.npy'), np.array(32)) # Number of Neurons for Edges
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'Neuron.npy'), np.array(192))     # Number of Neurons for Nodes
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'MdlType.npy'), np.array(8))#0:"GCN2",1:"GCN5",2:"GCN6",3:"GCN3",4:"SGA2",5:"SGA3",6:"SGA4",7:"SGA5",8:"GCN4",9:"MLP"
    np.save(os.path.join(temp, 'GATHead.npy'), np.array(0)) # Number of Heads of GAT Model
else:
    if args.enc == "Y":
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN64GAT3H2LR.0001Enc1.pth')
    else:
        file_pathML = os.path.join(os.getcwd(),'Sample_Datasets','Angl','ENN32BNN64GAT3H2LR.0001Enc0.pth')
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'EdgeNeuron.npy'), np.array(32)) # Number of Neurons for Edges
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'Neuron.npy'), np.array(64))     # Number of Neurons for Nodes
    np.save(os.path.join(os.getcwd(),'Sample_Datasets','tmp_files', 'MdlType.npy'), np.array(5))#0:"GCN2",1:"GCN5",2:"GCN6",3:"GCN3",4:"SGA2",5:"SGA3",6:"SGA4",7:"SGA5",8:"GCN4",9:"MLP"
...
# PMU Coverage
if os.path.exists(os.path.join(os.getcwd(), 'Sample_Datasets','Angl','PMUmiss.lst')):
    edge_index0 = np.genfromtxt(os.path.join(file_path22, 'EdgeIndex.csv'), delimiter=',')
    file_nameLST = os.path.join(os.getcwd(),'Sample_Datasets','Angl','PMUmiss.lst')
...
if OAJPE==1:
    if int(args.imp) == 2 and int(args.per) > 0:
        pmuerr_std(100)
    else:
        pmuerr_std(1  )
