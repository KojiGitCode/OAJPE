...
def btn2_click(): #TRAINING#
    ...
    class MyOwnDataset(InMemoryDataset):
        ...
        def process(self):
            if os.path.exists(os.path.join(temp, "data", "processed", "datas.pt")):
                ...
                #f str(args_mdl) == "MLP": #fault duration is considered as a node feature
                ...
                if   int(args_Ainx) == 0:
                    ...
                    LBL0   = np.genfromtxt(os.path.join(file_path22, 'Labels_GCN.Angle2.inx.csv'),  delimiter=',') #Amax
    ...
    max_acc_val_loss, min_loss_val_acc = 100, 0
    ...
    for epoch in range(int(args_epoch)):#15001
        ...
        if int(args_Ainx) < 5: # Vinx (node classificataion)
        ...
        if batch_val_loss > val_loss_stock:
            ...
            if save_flag == 0:
                ...
                if int(args_logits) == 0:
                    ...
                    save_flag   = 1
            #lif save_flag ==1 and min_val_loss > batch_val_loss:
            if save_flag ==1 and min_val_loss > batch_val_loss:
                ...
                if int(args_logits) == 0:
                    ...
                    torch.save(model, os.path.join(temp, args_inx, 'model_enc_ay.pth'))
            #lif save_flag ==1 and batch_val_acc > max_val_acc:
    ...
    test_acc, max_acc_model, min_loss_model = 0, 0, 0
    if NumGraph < 1000:
    ...
    if max_acc_model > min_loss_model and epoch_stock < epoch_stock2:
        ...
        if int(args_Ainx) < 5: # graph classification
            for kk in range (len(test_dataset)):#if prediction and label are not matched ..., kk denotes number of Testing dataset
                if not sc3[0][kk] == sc3[1][kk]:
                    ...
                    tee4e('{:.4g}'.format(kk).rjust(4),"/",len(test_dataset),",", ConsoleFile)
                    tee4e(formatted_indices[kk].zfill(6),",",N1_list[int(formatted_indices[kk])],", [", ConsoleFile)
                    for ii in range(dataset.num_classes):
                        tee2e('{:.3f}'.format(ttt[kk][ii]), ",", ConsoleFile)
                    tee ("]", ConsoleFile)
            ...
            for kk in range (len(test_dataset)):
                for jj in range(dataset[0].num_nodes):
                    ...
                    if not sc3[0][pp] == sc3[1][pp]:
                        ...
                        tee4e('{:.6g}'.format(pp).rjust(6),"/",len(test_dataset)*dataset[0].num_nodes,",", ConsoleFile)
                        tee4e(formatted_indices[kk].zfill(6),",",N1_list[int(formatted_indices[kk])],", [", ConsoleFile)
                        for ii in range(dataset.num_classes):
                            tee2e('{:.3f}'.format(ttt[pp][ii]), ",", ConsoleFile)
                        tee ("]", ConsoleFile)
                        MsmchList[0].append(N1_list[int(formatted_indices[kk])])
...
def Pref_Close(Flag):
    ...
    if Flag == 1:
        ...
        np.save(os.path.join(temp, 'MdlType.npy'),        np.array(6))     # GNN Model Type
        ...
        np.save(os.path.join(temp, 'Epochs.npy'),         np.array(4501))  # Number of Epochs
        np.save(os.path.join(temp, 'Neuron.npy'),         np.array(64))    # Number of Neurons
        np.save(os.path.join(temp, 'GATHead.npy'),        np.array(4))     # Number of heads of GAT
        np.save(os.path.join(temp, 'Batch.npy'),          np.array(504))   # Batch Size
        args_logits = 1                # Ord. enc.
...
file_path22 = os.path.join(os.getcwd(),'Sample_Datasets','Freq','TrainingDataset')
...
ini_dir = os.path.join(os.getcwd(),'Sample_Datasets','Freq','F1index')
IniFname = "GAT4H4LR.0001Enc1.pth"
